...
import { renderVerifyScreen } from './screens/verify.js';
...
export class Router {
  ...
  init() {
    // Check if directly loaded via /verify/:certId pathname
    if (window.location.pathname.startsWith('/verify/')) {
      const certParam = window.location.pathname.replace(/^\/verify\/?/, '');
      window.location.hash = '#verify' + (certParam ? '/' + certParam : '');
    }

    // Startup sequence requirement: App Open -> Splash screen (~2s) -> Login (or destination if logged in)
    // If application has not completed its initial boot, enforce #splash (unless direct public verify)
    if (!document.body.classList.contains('bootstrapped')) {
      if (!window.location.hash.startsWith('#verify') && window.location.hash !== '#splash') {
  ...
  handleRoute() {
    ...
    // Check authentication for protected screens (verify, splash, login are public)
    const token = localStorage.getItem('mine_ar_token');
    const userRole = localStorage.getItem('mine_ar_role') || this.state.userRole;

    if (!token && route !== 'splash' && route !== 'login' && route !== 'verify') {
    ...
    switch (route) {
      ...
      case 'certificate':
        ...
        break;
      case 'verify':
        renderVerifyScreen(this.container, this.state, subParam);
        break;
      case 'settings':
  ...
  updateBottomNav(route) {
    ...
    // Hide nav on splash, login, supervisor, verify, and immersive AR
    if (route === 'ar' || route === 'login' || route === 'splash' || route === 'supervisor' || route === 'verify') {
