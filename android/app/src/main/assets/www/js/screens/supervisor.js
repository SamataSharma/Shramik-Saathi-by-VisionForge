...
export async function renderSupervisorScreen(container, state) {
          ...
          <table class="roster-table" style="width: 100%; border-collapse: collapse; font-size: 13px;">
            <thead>
              <tr style="border-bottom: 2px solid #e2e8f0; text-align: left; color: #64748b; font-size: 11px; text-transform: uppercase;">
                ...
                <th style="padding: 12px 10px;">${t('issue_certificate')}</th>
                <th style="padding: 12px 10px; text-align: right;">${t('discover')}</th>
              </tr>
            </thead>
            <tbody>
              ${roster.map(w => {
                const isIssued = w.certificate_status === 'ISSUED';
                const certId = w.certificate_id || (w.id === 'sahnik01' ? 'CERT-DGMS-2026-8941' : (w.id === 'abhay01' ? 'CERT-DGMS-2026-8941' : ''));
                return `
                <tr data-drill-trainee="${w.id}" style="border-bottom: 1px solid #f1f5f9; cursor: pointer; transition: background 0.15s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='transparent'">
                  <td style="padding: 12px 10px;">
                    <div style="display: flex; align-items: center; gap: 10px;">
                      ...
                      <div>
                        ...
                        <div style="font-size: 11px; color: #64748b;">${w.role} • ID: <span style="font-family: monospace; font-weight: 700;">${w.id}</span></div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td style="padding: 12px 10px;">
                    <div style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
                      ${isIssued ? `
                        <span style="font-size: 10.5px; font-weight: 800; color: #065f46; background: #d1fae5; padding: 2px 7px; border-radius: 9999px;">
                          ✅ ${t('cert_issued')}
                        </span>
                        <button class="btn-worker-reissue-cert" data-wid="${w.id}" data-wname="${w.name}" data-wcomp="${w.competency}" title="Reissue updated certificate" style="padding: 5px 10px; font-size: 11px; font-weight: 700; background: #fef3c7; color: #92400e; border: 1px solid #fde68a; border-radius: 6px; cursor: pointer;">
                          🔄 ${t('reissue_certificate')}
                        </button>
                        <button class="btn-worker-view-cert" data-certid="${certId}" data-wid="${w.id}" title="Open Verification" style="padding: 5px 10px; font-size: 11px; font-weight: 700; background: #e0f2fe; color: #0369a1; border: 1px solid #bae6fd; border-radius: 6px; cursor: pointer;">
                          👁️ Verify
                        </button>
                      ` : `
                        <button class="btn-worker-issue-cert" data-wid="${w.id}" data-wname="${w.name}" data-wcomp="${w.competency}" style="padding: 6px 12px; font-size: 11.5px; font-weight: 800; background: linear-gradient(135deg, #059669, #10b981); color: #ffffff; border: none; border-radius: 6px; cursor: pointer; display: inline-flex; align-items: center; gap: 5px; box-shadow: 0 2px 4px rgba(16,185,129,0.25);">
                          📜 ${t('issue_certificate')}
                        </button>
                      `}
                    </div>
                  ...
                  <td style="padding: 12px 10px; text-align: right;">
                    <button class="btn-primary btn-drill-action" data-drill-target="${w.id}" style="padding: 6px 12px; font-size: 11.5px; font-weight: 700; border-radius: 6px;">
                      Drill Down ➔
                    </button>
                  </td>
                </tr>
  ...
  // Drilldown Click on explicit buttons
  container.querySelectorAll('.btn-drill-action').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      const tid = btn.getAttribute('data-drill-target');
      openTraineeDrilldownModal(tid, state);
    });
  });

  // Individual Worker Certificate Issue Action (Per-Worker Button)
  container.querySelectorAll('.btn-worker-issue-cert').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      e.stopPropagation();
      const wid = btn.getAttribute('data-wid');
      const wname = btn.getAttribute('data-wname');
      const wcomp = btn.getAttribute('data-wcomp');

      const confirmed = confirm(
        `CONFIRM STATUTORY CERTIFICATE ISSUANCE:\n\n` +
        `Worker: ${wname} (ID: ${wid})\n` +
        `Competency Score: ${wcomp}%\n` +
        `Issuing Authority: DGMS Authorized Safety Supervisor\n` +
        `Date: ${new Date().toISOString().split('T')[0]}\n\n` +
        `Are you sure you want to issue an official DGMS Competency Certificate to this worker?`
      );
      if (!confirmed) return;
      ...
      try {
        const resp = await fetch('/api/v1/supervisor/certificates/issue', {
          ...
          body: JSON.stringify({
            ...
            notes: `DGMS Certificate officially issued for ${wname} (${wid}) at ${wcomp}% competency score.`
          })
        });
        ...
        if (resp.ok) {
          ...
          alert(
            `✅ DGMS Certificate Issued Successfully!\n\n` +
            `Worker: ${wname} (${wid})\n` +
            `Certificate ID: ${res.certificate.cert_id}\n` +
            `Verification URL: ${window.location.origin}/#verify/${res.certificate.cert_id}\n\n` +
            `Logged to DGMS Statutory Audit Log.`
          );
          renderSupervisorScreen(container, state);
        } else {
          const err = await resp.json();
          if (resp.status === 409) {
            const reconfirm = confirm(`A certificate already exists for ${wname} (${err.detail}).\n\nWould you like to reissue and update the certificate?`);
            if (reconfirm) {
              await reissueCertificateForWorker(wid, wname, wcomp, container, state);
              return;
            }
          } else {
            alert(`Issuance Failed: ${err.detail || 'Access Denied'}`);
          }
          btn.disabled = false;
          btn.textContent = `📜 ${t('issue_certificate')}`;
        }
      } catch (err) {
  ...
  // Individual Worker Certificate Reissue Action
  container.querySelectorAll('.btn-worker-reissue-cert').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      e.stopPropagation();
      const wid = btn.getAttribute('data-wid');
      const wname = btn.getAttribute('data-wname');
  ...
  // Individual Worker Certificate View / Verify Action
  container.querySelectorAll('.btn-worker-view-cert').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      ...
      if (certId) {
        ...
        window.location.hash = `#certificate?trainee=${wid}`;
      }
    });
  });
}

// ----------------- REISSUE CERTIFICATE HELPER ----------------- //

async function reissueCertificateForWorker(wid, wname, wcomp, container, state) {
  const confirmed = confirm(
    `CONFIRM REISSUANCE:\n\n` +
    `Worker: ${wname} (Worker ID: ${wid})\n` +
    `Current Competency: ${wcomp}%\n\n` +
    `This will re-issue the official DGMS Certificate with updated assessment telemetry and timestamp, superseding previous record.\n\n` +
    `Proceed with reissuing?`
  );
  if (!confirmed) return;
  ...
  try {
    const resp = await fetch('/api/v1/supervisor/certificates/issue', {
      ...
      body: JSON.stringify({
        ...
        supervisor_id: 'supervisor01',
        is_reissue: true,
        notes: `DGMS Certificate REISSUED for ${wname} (${wid}) with updated telemetry.`
      })
    });
    ...
    if (resp.ok) {
      const res = await resp.json();
      alert(
        `✅ DGMS Certificate Reissued Successfully!\n\n` +
        `Worker: ${wname} (${wid})\n` +
        `Certificate ID: ${res.certificate.cert_id}\n` +
        `Verification URL: ${window.location.origin}/#verify/${res.certificate.cert_id}\n\n` +
        `Updated in DGMS Statutory Audit Log.`
      );
      renderSupervisorScreen(container, state);
    } else {
      const err = await resp.json();
      alert(`Reissuance Failed: ${err.detail || 'Access Denied'}`);
    }
  } catch (err) {
    alert(`Error reissuing certificate: ${err.message}`);
  }
}
    ...
    try {
      const resp = await fetch('/api/v1/supervisor/retraining/assign', {
        ...
        body: JSON.stringify({
          ...
          notes: 'Assigned via SHRAMIK SAATHI Safety Training Control Center.'
        })
      });
...
async function openTraineeDrilldownModal(traineeId, state) {
        ...
        <div id="tab-cert" class="modal-tab-content" style="display: none;">
          <div style="background: #ffffff; border: 1.5px solid #0284c7; border-radius: 14px; padding: 20px; text-align: center;">
            ...
            <p style="font-size: 12.5px; color: #64748b; margin: 0 0 16px 0;">
              Only an authenticated supervisor can authorize, issue, or revoke competency certificates.
            </p>
            ...
            <div style="background: #f8fafc; padding: 14px; border-radius: 10px; text-align: left; font-size: 12.5px; line-height: 1.6; margin-bottom: 18px; border: 1px solid #e2e8f0;">
              <div>Trainee: <strong>${trainee.name}</strong> (${trainee.role} • ID: ${trainee.id})</div>
              ...
              ${trainee.certificate && trainee.certificate.cert_id ? `
                <div style="margin-top: 8px; padding-top: 8px; border-top: 1px dashed #cbd5e1;">
                  <div>Certificate ID: <strong style="font-family: monospace; color: #0f172a;">${trainee.certificate.cert_id}</strong></div>
                  <div>Issued Date: <strong>${trainee.certificate.issue_date || 'N/A'}</strong></div>
                  <div>Record Status: <strong style="color: ${trainee.certificate.status === 'VALID' ? '#10b981' : (trainee.certificate.status === 'REVOKED' ? '#ef4444' : '#f59e0b')};">${trainee.certificate.status}</strong></div>
                </div>
              ` : ''}
            </div>
            ...
            <div style="display: flex; flex-direction: column; gap: 8px;">
              ${trainee.certificate && trainee.certificate.cert_id ? `
                <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                  <button id="btn-sup-reissue-cert" class="btn-primary" style="flex: 1; padding: 12px; font-size: 13.5px; font-weight: 800; background: linear-gradient(135deg, #d97706, #b45309); border: none; border-radius: 8px; color: #ffffff; cursor: pointer;">
                    🔄 Reissue Certificate
                  </button>
                  <a href="#verify/${trainee.certificate.cert_id}" id="btn-sup-open-verify" style="flex: 1; padding: 12px; font-size: 13.5px; font-weight: 800; background: #0284c7; border: none; border-radius: 8px; color: #ffffff; text-decoration: none; display: flex; align-items: center; justify-content: center;">
                    👁️ Open QR Verification
                  </a>
                </div>
                ${trainee.certificate.status === 'VALID' ? `
                  <button id="btn-sup-revoke-cert" style="padding: 10px; font-size: 12px; font-weight: 700; background: #fee2e2; color: #991b1b; border: 1px solid #fca5a5; border-radius: 8px; cursor: pointer;">
    ...
    try {
      ...
      if (resp.ok) {
        ...
        fb.innerHTML = `
          ...
          Verification URL: <a href="#verify/${certData.certificate.cert_id}" style="color: #0369a1; font-weight: 800;">/#verify/${certData.certificate.cert_id}</a><br>
          QR Verification Hash Generated & Logged to DGMS Audit Trail.
        `;
        if (slot.querySelector('#btn-sup-issue-cert')) slot.querySelector('#btn-sup-issue-cert').disabled = true;
      } else {
        const err = await resp.json();
  ...
  // Reissue Certificate Handler
  slot.querySelector('#btn-sup-reissue-cert')?.addEventListener('click', async () => {
    const confirmed = confirm(
      `CONFIRM CERTIFICATE REISSUANCE:\n\n` +
      `Trainee: ${trainee.name} (ID: ${trainee.id})\n` +
      `This will update the certificate with fresh assessment records.\n\n` +
      `Proceed with reissuing?`
    );
    if (!confirmed) return;
    ...
    try {
      const resp = await fetch('/api/v1/supervisor/certificates/issue', {
        ...
        body: JSON.stringify({
          ...
          supervisor_id: 'supervisor01',
          is_reissue: true,
          notes: `Certificate reissued for ${trainee.name} with updated assessment records.`
        })
      });
      ...
      if (resp.ok) {
        ...
        fb.style.color = '#065f46';
        fb.innerHTML = `
          <strong>✅ Certificate Reissued Successfully!</strong><br>
          Certificate ID: <strong>${certData.certificate.cert_id}</strong><br>
          Verification URL: <a href="#verify/${certData.certificate.cert_id}" style="color: #0369a1; font-weight: 800;">/#verify/${certData.certificate.cert_id}</a>
        `;
      } else {
        const err = await resp.json();
        fb.style.background = '#fee2e2';
        fb.style.color = '#991b1b';
        fb.textContent = `Reissuance Failed: ${err.detail || 'Access Denied'}`;
      }
    } catch (e) {
      alert("Reissuance failed: " + e.message);
    }
  });

  // Revoke Certificate Handler
  slot.querySelector('#btn-sup-revoke-cert')?.addEventListener('click', async () => {
    const reason = prompt("Enter statutory reason for certificate revocation:", "Periodic competency reassessment failure or safety protocol infraction");
    if (!reason) return;

    try {
      const resp = await fetch('/api/v1/supervisor/certificates/revoke', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('mine_ar_token') || 'test-sup-token'}`
        ...
        body: JSON.stringify({
          cert_id: trainee.certificate.cert_id,
          supervisor_id: 'supervisor01',
          reason: reason
        })
      });
      ...
      if (resp.ok) {
        fb.style.background = '#fee2e2';
        fb.style.color = '#991b1b';
        fb.innerHTML = `<strong>⚠️ Certificate Revoked!</strong> Status marked as REVOKED in DGMS Registry.`;
      } else {
        const err = await resp.json();
        alert(`Revocation Failed: ${err.detail || 'Access Denied'}`);
      }
    } catch (e) {
