...
def seed_all_data(conn):
    ...
    # Verify if new trainees and all 5 scenarios exist
    cur.execute("SELECT COUNT(*) FROM workers WHERE id = 'anik01'")
    has_anik = cur.fetchone()[0] > 0
    cur.execute("SELECT COUNT(*) FROM scenarios WHERE id = 'sim_roof_strata'")
    has_scenarios = cur.fetchone()[0] > 0

    if not has_anik or not has_scenarios:
        ...
        # 4. Scenarios (All 5 Official Vocational Simulations)
        cur.execute("DELETE FROM scenarios")
        scenarios_data = [
            (
                "sim_fire_explosion",
                "Conveyor Belt Friction & Spontaneous Heating",
                "Fire & Explosion Response",
                "Thermal Combustion & Toxic Smoke",
                "Trunk Belt Conveyor Gallery",
                "Medium",
                "Friction between a seized conveyor roller idler and vulcanized rubber belt has generated localized spontaneous heating with glowing coal fines and Carbon Monoxide smoke.",
                60,
                "dcp_extinguisher",
                "intake",
            ...
            (
                "sim_gas_confined",
                "Multi-Gas Detection & Confined Space Entry",
                "Gas Leak & Confined Space",
                "Inflammable Gas Accumulation & Oxygen Deficiency",
                "Continuous Miner Ingress Heading",
                "Hard",
                "Entering a remote coal seam heading requires multi-gas testing, personal protective verification, and continuous communication with your assigned safety buddy.",
                60,
                "multi_gas_detector",
                "intake",
                json.dumps(["comms_check", "ppe_check", "gas_sampling", "isolate_power", "retreat_upwind"])
            ),
            (
                "sim_machinery_safety",
                "Continuous Miner & Trailing Cable Safety",
                "Machinery Safety",
                "Mechanical Pinch Points & High-Voltage Cable Arcing",
                "Heavy Machine Transfer Substation",
                "Hard",
                "A trailing cable to the continuous miner cutter head is showing mechanical abrasion near the haulage crossover. Trainee must verify isolation, safe clearance, and lockout before approach.",
                60,
                "loto_lockout_kit",
                "clearance_zone",
                json.dumps(["hazard_inspect", "boom_clearance", "isolate_breaker", "apply_loto", "pull_cord_test"])
            ),
            (
                "sim_emergency_evac",
                "Subsurface Evacuation & Lifeline Protocol",
                "Emergency Evacuation",
                "Visibility Loss & Airway Smoke Contamination",
                "Main Seam Trunk Escapeway",
                "Hard",
                "Fire alarm sirens sound along the panel. Visibility is deteriorating. Trainee must don self-rescuer respiratory pack, locate tactile lifeline, and lead retreat to the intake shaft or refuge bay.",
                60,
                "scsr_oxygen_pack",
                "intake_lifeline",
                json.dumps(["don_scsr", "seal_mask", "follow_lifeline", "latch_airlock", "enter_refuge"])
            ),
            (
                "sim_roof_strata",
                "Strata Control & Roof Fall Prevention",
                "Strata Control & Roof Support",
                "Sandstone Delamination & Unstable Spalling Rocks",
                "Active Extraction Working Seam",
                "Medium",
                "Audible cracking and spalling rocks detected along the sandstone bedding plane at the coal extraction face. Trainee must execute roof sounding, check support props, and enforce safe stand-off.",
                60,
                "sounding_rod",
                "supported_roadway",
                json.dumps(["acoustic_detect", "sound_and_tap", "prop_inspect", "mark_boundary", "withdraw_alert"])
            )
        ]
        cur.executemany("""
        ...
        # 8. AR Scenario Attempts (Across All 5 Official Simulations)
        cur.execute("DELETE FROM attempts")
        ar_attempts = [
            # 1. Fire & Explosion Response
            ("att_ar_anik_1", "anik01", "sim_fire_explosion", "2026-09-10T15:20:00Z", 48000, 6200, "dcp_extinguisher", "intake",
             json.dumps(["pull", "aim", "squeeze", "sweep"]), json.dumps([]), 55, 85.0, 80.0, 88.0, 85.0, 84.5, 1),
            ("att_ar_sam_1", "samata01", "sim_fire_explosion", "2026-09-11T10:30:00Z", 36000, 3400, "dcp_extinguisher", "intake",
             json.dumps(["pull", "aim", "squeeze", "sweep"]), json.dumps([]), 75, 95.0, 92.0, 95.0, 95.0, 93.8, 1),
            ("att_ar_sahnik_1", "sahnik01", "sim_fire_explosion", "2026-09-12T11:00:00Z", 65000, 12500, "water_extinguisher", "return",
            ...
            ("att_ar_anik_2", "anik01", "sim_gas_confined", "2026-09-11T16:00:00Z", 52000, 4800, "multi_gas_detector", "intake",
             json.dumps(["comms_check", "ppe_check", "gas_sampling", "isolate_power", "retreat_upwind"]), json.dumps([]), 65, 92.0, 84.0, 90.0, 90.0, 89.2, 1),
            ("att_ar_sam_2", "samata01", "sim_gas_confined", "2026-09-12T14:10:00Z", 42000, 3100, "multi_gas_detector", "intake",
             json.dumps(["comms_check", "ppe_check", "gas_sampling", "isolate_power", "retreat_upwind"]), json.dumps([]), 80, 96.0, 90.0, 95.0, 95.0, 94.2, 1),

            # 3. Machinery Safety & LOTO
            ("att_ar_ark_1", "arkadip01", "sim_machinery_safety", "2026-09-11T09:40:00Z", 44000, 4200, "loto_lockout_kit", "clearance_zone",
             json.dumps(["hazard_inspect", "boom_clearance", "isolate_breaker", "apply_loto", "pull_cord_test"]), json.dumps([]), 70, 94.0, 88.0, 92.0, 90.0, 91.5, 1),
            ("att_ar_shamb_1", "shambhavi01", "sim_machinery_safety", "2026-09-12T15:30:00Z", 62000, 8500, "loto_lockout_kit", "clearance_zone",
             json.dumps(["hazard_inspect", "boom_clearance"]), json.dumps(["Delayed pull-cord trip switch test"]), 30, 65.0, 60.0, 70.0, 65.0, 64.8, 0),

            # 4. Emergency Evacuation
            ("att_ar_abhay_1", "abhay01", "sim_emergency_evac", "2026-09-08T09:00:00Z", 28000, 2600, "scsr_oxygen_pack", "intake_lifeline",
             json.dumps(["don_scsr", "seal_mask", "follow_lifeline", "latch_airlock", "enter_refuge"]), json.dumps([]), 90, 98.0, 96.0, 98.0, 97.0, 97.2, 1),
            ("att_ar_anik_3", "anik01", "sim_emergency_evac", "2026-09-12T17:15:00Z", 39000, 3900, "scsr_oxygen_pack", "intake_lifeline",
             json.dumps(["don_scsr", "seal_mask", "follow_lifeline", "latch_airlock", "enter_refuge"]), json.dumps([]), 75, 90.0, 86.0, 92.0, 90.0, 89.5, 1),

            # 5. Strata Control & Roof Support
            ("att_ar_sam_3", "samata01", "sim_roof_strata", "2026-09-12T16:20:00Z", 41000, 3500, "sounding_rod", "supported_roadway",
             json.dumps(["acoustic_detect", "sound_and_tap", "prop_inspect", "mark_boundary", "withdraw_alert"]), json.dumps([]), 80, 95.0, 90.0, 95.0, 92.0, 93.3, 1),
            ("att_ar_sahnik_2", "sahnik01", "sim_roof_strata", "2026-09-12T18:00:00Z", 58000, 9200, "sounding_rod", "supported_roadway",
             json.dumps(["acoustic_detect", "sound_and_tap"]), json.dumps(["Advanced under drummy strata before prop check"]), 25, 55.0, 50.0, 60.0, 55.0, 55.5, 0)
        ]
        cur.executemany("""
