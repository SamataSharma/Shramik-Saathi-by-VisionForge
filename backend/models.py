...
class IssueCertificateRequest(BaseModel):
    trainee_id: str
    supervisor_id: str
    is_reissue: Optional[bool] = False
    notes: Optional[str] = "Approved after complete competency audit and simulation review."

class RevokeCertificateRequest(BaseModel):
    cert_id: str
    supervisor_id: str
    reason: Optional[str] = "Statutory reassessment or safety protocol infraction"
...
class LessonQuizSubmission(BaseModel):
    ...
    question: str
    selected_answer: str
    time_taken_sec: Optional[float] = 5.0

class AssignRetrainingRequest(BaseModel):
    trainee_id: str
    supervisor_id: Optional[str] = "supervisor01"
    modules: List[str]
    notes: Optional[str] = "Mandatory retraining assigned by supervisor."
